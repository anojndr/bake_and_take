...
$stmt = mysqli_prepare($conn, 'SELECT user_id, first_name, email, is_admin FROM users WHERE email = ? LIMIT 1');
if (!$stmt) {
    error_log('Forgot password error: ' . mysqli_error($conn));
    redirect('../index.php?page=login', $genericMessage, 'info');
}
...
$update = mysqli_prepare($conn, 'UPDATE users SET password_reset_token_hash = ?, password_reset_expires_at = ? WHERE user_id = ?');
if (!$update) {
    error_log('Forgot password error: ' . mysqli_error($conn));
    redirect('../index.php?page=login', $genericMessage, 'info');
}
mysqli_stmt_bind_param($update, "ssi", $tokenHash, $expiresAt, $user['user_id']);
