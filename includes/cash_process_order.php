 ...
 * Processes orders to be paid via Cash on Pickup
 */
session_start();
...
$orderNumber = 'CS' . strtoupper(substr(md5(uniqid(rand(), true)), 0, 6));
...
if ($pdo) {
    try {
        ...
        // Insert order with Cash payment method
        $stmt = $pdo->prepare("
            INSERT INTO orders (
            ...
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'confirmed', 'cash', 'pending', NOW())
        ");
        ...
        $orderSubject = "Bake & Take - Order Confirmation #{$orderNumber} (Cash on Pickup)";
        $orderBody = "
            ...
            <p style='background: #D4EDDA; padding: 15px; border-radius: 8px; border-left: 4px solid #28A745;'>
                ...
                Please prepare the exact amount when picking up your order. Payment will be collected at our store.
            </p>
            <p>Your order will be ready for pickup at our store.</p>
        ...
        // Send email to Admin
        $adminSubject = "New Cash Order #{$orderNumber}";
        $adminBody = "
            <h2>New Cash Order Received</h2>
            <p style='background: #D4EDDA; padding: 15px; border-radius: 8px; border-left: 4px solid #28A745;'>
                <strong>💵 Cash on Pickup Order</strong><br>
                Payment to be collected at the store.
            </p>
        ...
        $pdo->rollBack();
        error_log("Cash order processing error: " . $e->getMessage());
